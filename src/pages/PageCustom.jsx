...
export default function PageCustom() {
  ...
  const pageIndex = data.customPages.findIndex(p => p.id === pageId);
  const page = data.customPages[pageIndex] || { title: `フリーページ ${pageId - 10}`, content: '' };

  const handleChange = (field, value) => {
    const newPages = [...data.customPages];
