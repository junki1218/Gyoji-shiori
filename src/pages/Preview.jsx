  ...
  return (
    <div className="animation-fade-in">
      <div className="page-header flex justify-between items-center hide-on-print">
        <div>
          <h1>完成プレビュー</h1>
          <p className="text-muted">この画面を印刷（PDF保存）して「しおり」として配布できます。</p>
        </div>
      ...
      <div className="print-container">
        ...
        <div className="shiori-page cover-page">
          <h1 className="title">{data.dateTime.title || '修学旅行のしおり'}</h1>
          <div className="info">
            <p>日程：{data.dateTime.startDate} 〜 {data.dateTime.endDate}</p>
            <p>行き先：{data.destination.name}</p>
          </div>
          <div className="goals-box">
            <h3>スローガン</h3>
            <p>{data.goals.classGoal || '（未設定）'}</p>
            <h3>個人の目標</h3>
            <p>{data.goals.personalGoal || '（未設定）'}</p>
          </div>
        </div>
          ...
          <div className="section">
            ...
            <table className="schedule-table">
              ...
              <tbody>
                ...
                )) : <tr><td colSpan="3" className="text-center text-muted">スケジュールがありません</td></tr>}
              </tbody>
            </table>
          </div>
              ...
              <ul className="belongings-list">
                {data.belongings.length > 0 ? data.belongings.map(item => (
                  <li key={item.id}>
                    ...
                    {item.name} {item.isEssential && <strong>(必須)</strong>}
                  </li>
                )) : <p className="text-muted">持ち物がありません</p>}
              </ul>
            </div>
            <div className="section">
              <h2>宿泊・座席</h2>
              <div className="info-box">
                <p><strong>部屋番号:</strong> {data.room.roomNumber}</p>
                <p><strong>同室メンバー:</strong> {data.room.members.join('、')}</p>
              </div>
              <div className="info-box mt-2">
                <p><strong>乗り物:</strong> {data.seats.transportType === 'bus' ? 'バス' : '電車'}</p>
                <p className="text-sm text-muted">座席は別途確認</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="shiori-page">
          <div className="section">
            ...
            <div className="roles-grid">
              ...
              )) : <p className="text-muted">役割がありません</p>}
            </div>
          </div>
